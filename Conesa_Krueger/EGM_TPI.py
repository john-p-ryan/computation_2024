from EGM_ss import OLGModel, constrained_c_root, get_prices, get_aggregates, solve_SS
...
def solve_HH_transition(OLG, r_vec, w_vec, b_vec):
    ...
    for j in range(OLG.N - OLG.J_r - 1, -1, -1):
        ...
        r_next = r_vec[t+1]
        b = b_vec[t]
        c_r = OLG.inv_mu_c(OLG.beta * (1+r_next) * OLG.mu_c(c_policy_r[j+1, :]))
    ...
    for z_index, z in enumerate(OLG.z_grid):
        ...
        c_w = OLG.inv_mu_c(OLG.beta * (1 + r_vec[t+1]) * OLG.mu_c(c_policy_r[0,:]))
        ...
        for a_k in range(OLG.na):
            for z in range(OLG.nz):
                for z_next in range(OLG.nz):
                    ...
                    if a_next <= OLG.a_min:
                        h_w_next[j, 0, z_next] += h_w_init[j-1, a_k, z] * OLG.pi[z, z_next] / (1 + OLG.n)
                    elif a_next >= OLG.a_max:
                        h_w_next[j, -1, z_next] += h_w_init[j-1, a_k, z] * OLG.pi[z, z_next] / (1 + OLG.n)
                    else:
                        idx = np.searchsorted(OLG.a_grid, a_next)
                        weight_high = (a_next - OLG.a_grid[idx-1]) / (OLG.a_grid[idx] - OLG.a_grid[idx-1])
                        weight_low = 1 - weight_high
                        h_w_next[j, idx-1, z_next] += weight_low * h_w_init[j-1, a_k, z] * OLG.pi[z, z_next] / (1 + OLG.n)
                        h_w_next[j, idx, z_next] += weight_high * h_w_init[j-1, a_k, z] * OLG.pi[z, z_next] / (1 + OLG.n)
    ...
    for a_k in range(OLG.na):
        for z in range(OLG.nz):
            ...
            if a_next <= OLG.a_min:
                ...
                weight_high = (a_next - OLG.a_grid[idx-1]) / (OLG.a_grid[idx] - OLG.a_grid[idx-1])
                weight_low = 1 - weight_high
                h_r_next[0, idx-1] += weight_low * h_w_init[-1, a_k, z] / (1 + OLG.n)
                h_r_next[0, idx] += weight_high * h_w_init[-1, a_k, z] / (1 + OLG.n)
    ...
    for j in range(1, OLG.N-OLG.J_r+1):
        for a_k in range(OLG.na):
            ...
            if a_next <= OLG.a_min:
                ...
                weight_high = (a_next - OLG.a_grid[idx-1]) / (OLG.a_grid[idx] - OLG.a_grid[idx-1])
                weight_low = 1 - weight_high
                h_r_next[j, idx-1] += weight_low * h_r_init[j-1, a_k] / (1 + OLG.n)
                h_r_next[j, idx] += weight_high * h_r_init[j-1, a_k] / (1 + OLG.n)
...
def get_cross_section(OLG, a_w_mat, a_r_mat, l_mat, t):
    ...
    for j in range(OLG.N-OLG.J_r+1):
        cohort_t = t - (OLG.J_r-1+j)  # The time period when this cohort was born
...
@njit
def SolveTransitionPath(OLG, T, K_0, L_0, h_w_0, h_r_0, tol=1e-5, maxiter=200, rho=.1):
    # Solve for steady state
    K_ss, L_ss, r_ss, w_ss, b_ss = solve_SS(OLG, tol=tol, maxiter=maxiter, rho=rho, K0=K_0, L0=L_0)
    ...
    while error > tol:
        ...
        # Extend price paths
        r_extended = np.concatenate([np.repeat(r_0, OLG.N), r_path, np.repeat(r_ss, OLG.N)])
        w_extended = np.concatenate([np.repeat(w_0, OLG.N), w_path, np.repeat(w_ss, OLG.N)])
        b_extended = np.concatenate([np.repeat(b_0, OLG.N), b_path, np.repeat(b_ss, OLG.N)])
        ...
        for t in range(T):
            ...
            h_w, h_r = iterate_distribution(OLG, h_w, h_r, a_policies_w, a_policies_r)

            # Compute aggregates
            K_path_next[t], L_path_next[t] = get_aggregates(OLG, h_w, h_r, l_policies)

        # Compute error
        error = max(np.max(np.abs(K_path_next - K_path)), np.max(np.abs(L_path_next - L_path)))
        ...
        if error < tol:
            ...
            for t in range(T):
                r_path[t], w_path[t], b_path[t] = get_prices(OLG, K_path[t], L_path[t])
