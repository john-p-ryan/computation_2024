...
class OLGModel:
    ...
    def u(self, c, l):
        return (c**self.gamma * (1 - l) ** (1 - self.gamma)) ** (
            1 - self.sigma
        ) / (1 - self.sigma)

    def V_induction(self, r=0.05, w=1.05, b=0.2):
        ...
        for z_index, z in enumerate(self.z_grid):
            ...
            for a_index, a in enumerate(self.a_grid):
                ...
                for ap_index, ap in enumerate(self.a_grid):
                    l = (
                        self.gamma * (1 - self.theta) * e * w
                        - (1 - self.gamma) * (a * (1 + r) - ap)
                    ) / (
                        (1 - self.theta) * w * e
                    )  # labor supply given a'
                    l = max(0, min(1, l))  # enforce bounds
                    c = (
                        (1 + r) * a + w * e * (1 - self.theta) * l - ap
                    )  # consumption given a'
                    if c > 0:  # check for positivity
                        val = (
                            self.u(c, l) + self.beta * V_r[0, ap_index]
                        )  # compute value function
                        if val > candidate_max:  # check for new max value
                            candidate_max = val  # update max value
                            ...
                            l_w[-1, a_index, z_index] = (
                                l  # update labor supply
                            )
                            u_w[-1, a_index, z_index] = self.u(
                                c, l
                            )  # update utility function

                V_w[-1, a_index, z_index] = (
            ...
            for z_index, z in enumerate(self.z_grid):
                ...
                for a_index, a in enumerate(self.a_grid):
                    ...
                    for ap_index, ap in enumerate(
                        ...
                        if c > 0:  # check for positivity
                            val = self.u(c, l) + self.beta * np.dot(
                                self.pi[z_index], V_w[j + 1, ap_index]
                            )  # compute value function
                            if val > candidate_max:  # check for new max value
                                candidate_max = val  # update max value
                    ...
                    for zp_index in range(self.nz):
                        for ap_index, ap in enumerate(self.a_grid):
                            if ap == g_w[j - 1, a_index, z_index]:
                                F_w[j, ap_index, zp_index] += (
                                    F_w[j - 1, a_index, z_index]
                                    * self.pi[z_index, zp_index]
                                    / (1 + self.n)
                                )
            ...
            for ap_index, ap in enumerate(self.a_grid):
                for z_index in range(self.nz):
                    if ap == g_w[self.J_r - 2, a_index, z_index]:
                        F_r[0, a_index] += F_w[-1, a_index, z_index] / (
                            1 + self.n
                        )
    ...
    def market_clearing(
        self, tol=0.0001, max_iter=200, rho=0.02, K0=3.32, L0=0.34
    ):
