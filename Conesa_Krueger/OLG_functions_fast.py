...
@njit
def V_induction(OLG, r=.05, w=1.05, b=.2):
        ...
        for a_index, a in enumerate(OLG.a_grid):
            ...
            for ap_index, ap in enumerate(OLG.a_grid):     # loop over possible selections of a'
                ...
                if c > 0:                                  # check for positivity
                    val = OLG.u(c, 0) + OLG.beta * V_r[j + 1, ap_index]  # compute value function
                    if val > candidate_max:                                 # check for new max value
                        candidate_max = val                                 # update max value
                        g_r[j, a_index] = ap                                # update policy function
                        u_r[j, a_index] = OLG.u(c, 0)                      # update utility function
    ...
    for z_index, z in enumerate(OLG.z_grid):
        ...
        for a_index, a in enumerate(OLG.a_grid):
            ...
            for ap_index, ap in enumerate(OLG.a_grid):
                l = (OLG.gamma*(1 - OLG.theta)*e*w - (1-OLG.gamma)*(a*(1+r)-ap)) / ((1-OLG.theta)*w*e)  # labor supply given a'
                l = max(0, min(1, l))  # enforce bounds
                c = (1+r)*a + w*e*(1-OLG.theta)*l - ap  # consumption given a'
                if c > 0:  # check for positivity
                    val = OLG.u(c, l) + OLG.beta * V_r[0, ap_index]  # compute value function
                    if val > candidate_max:  # check for new max value
                        candidate_max = val  # update max value
                        g_w[-1, a_index, z_index] = ap  # update policy function
                        l_w[-1, a_index, z_index] = l  # update labor supply
                        u_w[-1, a_index, z_index] = OLG.u(c, l)  # update utility function
    ...
    for j in range(OLG.J_r - 3, -1, -1):  # age = J_r-2, iterate backwards
        for z_index, z in enumerate(OLG.z_grid):
            ...
            for a_index, a in enumerate(OLG.a_grid):
                ...
                # grid search
                for ap_index, ap in enumerate(OLG.a_grid):  # loop over vals of a'
                    l = (OLG.gamma*(1-OLG.theta)*e*w - (1-OLG.gamma)*(a*(1+r)-ap)) / ((1-OLG.theta)*w*e)  # labor supply given a'
                    l = max(0, min(1, l))
                    c = (1+r) * a + w * e * (1 - OLG.theta) * l - ap  # consumption given a'

                    if c > 0:  # check for positivity
                        val = OLG.u(c, l) + OLG.beta * np.dot(OLG.pi[z_index], V_w[j + 1, ap_index])  # compute value function
                        if val > candidate_max:  # check for new max value
                            candidate_max = val  # update max value
                            g_w[j, a_index, z_index] = ap  # update policy function
                            l_w[j, a_index, z_index] = l  # update labor supply
                            u_w[j, a_index, z_index] = OLG.u(c, l)  # update utility function
