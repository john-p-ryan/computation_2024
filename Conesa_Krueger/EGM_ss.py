...
@njit
def solve_HH_SS(OLG, r=0.05, w=1.05, b=0.2):
...
@njit
def SS_distribution(OLG, a_policy_w, a_policy_r):
...
@njit
def get_aggregates(OLG, h_w, h_r, l_w):
...
def get_prices(OLG, K, L):
    # compute prices implied by capital and labor supply
    r = OLG.alpha * (L / K) ** (1 - OLG.alpha) - OLG.delta
    w = (1 - OLG.alpha) * (K / L) ** OLG.alpha
    b = OLG.theta * w * L / np.sum(OLG.mu[OLG.J_r - 1 :])
    return r, w, b


@njit
def solve_SS(OLG, tol=0.0001, max_iter=200, rho=0.02, K0=3.32, L0=0.34):
    ...
    while error > tol:
        r, w, b = get_prices(OLG, K, L)
        a_w, a_r, _, _, l_w = solve_HH_SS(OLG, r, w, b)
        h_w, h_r = SS_distribution(OLG, a_w, a_r)
        K_new, L_new = get_aggregates(OLG, h_w, h_r, l_w)
