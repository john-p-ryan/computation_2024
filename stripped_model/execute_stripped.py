...
eta_end = np.linspace(ef[-1], 0.5, 67 - len(ef))
...
og = OLGModel(b_grid=b_grid, eta=eta, z_grid=np.array([1.0, 0.25]), gamma=2.5)


# %%
begin = time.time()
b_policy, c_policy, n_policy = solve_HH_SS(og, r=0.05, w=1.2)
end = time.time()
...
# %%
begin = time.time()
eqbm = solve_SS(og, tol=1e-5, max_iter=1000, rho=0.5)
end = time.time()
print(f"Time elapsed: {end-begin} seconds")
print("K, L, r, w: ")
print(eqbm)
...
b_transition, c_transition, n_transition = solve_HH_transition(
    og, r_vec, w_vec
)
...
# %%
og2 = OLGModel(
    b_grid=b_grid, eta=eta, z_grid=np.array([1.0, 0.25]), gamma=2.5, theta=0.25
)
K_0, L_0, r_0, w_0 = solve_SS(og2, tol=1e-5, max_iter=1000, rho=0.5)
...
T = og.S * 3
begin = time.time()
transition_path = solve_transition_path(
    og, T, K_0, L_0, h_0, tol=1e-4, maxiter=500, rho=0.5
)
end = time.time()
print(f"Time elapsed: {end-begin} seconds")
