...
from OLG_ss_stripped import OLGModel, c_root, get_prices, get_aggregates, solve_SS
        ...
        for b_index in range(OLG.nb):
            for z_index in range(OLG.nz):
                for z_next in range(OLG.nz):
                    ...
                    if b_next <= OLG.b_min:
                        h_next[s+1, 0, z_next] += h_init[s, b_index, z_index] * OLG.pi[z_index, z_next] / (1 + OLG.g_n)
                    elif b_next >= OLG.b_max:
                        h_next[s+1, -1, z_next] += h_init[s, b_index, z_index] * OLG.pi[z_index, z_next] / (1 + OLG.g_n)
                    else:
                        idx = np.searchsorted(OLG.b_grid, b_next)
                        weight_high = (b_next - OLG.b_grid[idx-1]) / (OLG.b_grid[idx] - OLG.b_grid[idx-1])
                        weight_low = 1 - weight_high
                        h_next[s+1, idx-1, z_next] += weight_low * h_init[s, b_index, z_index] * OLG.pi[z_index, z_next] / (1 + OLG.g_n)
                        h_next[s+1, idx, z_next] += weight_high * h_init[s, b_index, z_index] * OLG.pi[z_index, z_next] / (1 + OLG.g_n)
...
@njit
def solve_transition_path(OLG, T, K_0, L_0, h_0, tol=1e-5, maxiter=500, rho=0.25):
    # Solve for steady state
    K_ss, L_ss, r_ss, w_ss = solve_SS(OLG, tol=tol, max_iter=maxiter, rho=rho, K0=K_0, L0=L_0)
    ...
    while error > tol:
        ...
        for j in range(num_cohorts):
            ...
            b_mat[j], c_mat[j], n_mat[j] = solve_HH_transition(OLG, r_vec, w_vec)
