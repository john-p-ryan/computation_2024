...
def solve_HH_transition(OLG, r_vec, w_vec, BQ_vec):
    ...
    for z_index, z in enumerate(OLG.z_grid):
        ...
        for b_index, b in enumerate(b_grid):
            ...
            c = brentq(
                c_root, 0, (1 + r_vec[-1]) * b + coef + BQ_received[-1], args
            ).root
            n = OLG.n_from_c(c, coef)
            c_policy[-1, b_index, z_index] = c
            n_policy[-1, b_index, z_index] = n
            b_policy[-1, b_index, z_index] = (
                (1 + r_vec[-1]) * b + coef * n + BQ_received[-1] - c
            )
    ...
    for s in range(OLG.S - 2, -1, -1):
        ...
        for z_index, z in enumerate(OLG.z_grid):
            ...
            c = OLG.inv_mu_c(
                OLG.rho[s] * OLG.mu_b(b_grid)
                + (1 - OLG.rho[s]) * OLG.beta * (1 + r_next) * E_mu_c
            )
            n = OLG.n_from_c(c, coef)
...
def solve_transition_path(
    OLG, T, initial_state, tol=1e-5, maxiter=500, xi=0.25
):

    K_0, L_0, BQ_0, h_0 = initial_state

    # Solve for steady state
    K_ss, L_ss, BQ_ss, r_ss, w_ss = solve_SS(
        OLG, tol=tol, max_iter=maxiter, xi=xi, initial_guess=(K_0, L_0, BQ_0)
    )
    ...
    while error > tol:
        ...
        BQ_extended = np.concatenate(
            (np.repeat(BQ_0, OLG.S - 1), BQ_path, np.repeat(BQ_ss, OLG.S - 1))
        )
        ...
        error = max(
            ...
            np.max(np.abs(BQ_path_next - BQ_path)),
        )
