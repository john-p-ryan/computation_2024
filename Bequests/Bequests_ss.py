...
@njit
def solve_HH_SS(OLG, r=0.05, w=1.05, BQ=1.0):
    ...
    for s in range(OLG.S - 2, -1, -1):
        for z_index, z in enumerate(OLG.z_grid):
            ...
            c = OLG.inv_mu_c(
                ...
                + (1 - OLG.rho[s]) * OLG.beta * (1 + r) * E_mu_c
            )
            n = OLG.n_from_c(c, coef)
...
@njit
def solve_SS(
    OLG, tol=0.0001, max_iter=200, xi=0.1, initial_guess=(1.0, 0.5, 0.1)
):
