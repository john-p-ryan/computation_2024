...
b_ellip, upsilon = elp.fit_ellip_CFE(
    ellip_init, cfe_params, l_tilde, ellip_graph
)
...
# Don't compute steady-state, get it from pickle
    ...
    if (not ss_vars_exst) or (not ss_args_exst):
        ...
        err_msg = (
            "ERROR: The SS output files do not exist and "
        ...
        if args_same:
            ...
            err_msg = (
                "ERROR: Current ss_args are not equal to the "
...
if TPI_solve:
    ...
    init_wgts = (1.5 - 0.87) / (S - 1) * (np.linspace(1, S, S) - 1) + 0.87
