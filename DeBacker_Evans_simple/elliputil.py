...
def gen_ellip(h, k, a, b, mu, graph=True):
    ...
    if graph:
        ...
        ax.add_patch(e1)
        plt.plot(
            xvec, yvec, color="r", linewidth=4, label="Upper-right quadrant"
        )
...
def fit_ellip_CFE(ellip_init, cfe_params, l_tilde, graph):
    ...
    if ellip_params.success:
        ...
        if graph:
            ...
            plt.grid(b=True, which="major", color="0.65", linestyle="-")
            plt.title(
                "CFE marginal utility versus fitted elliptical", fontsize=20
            )
            ...
            output_path = os.path.join(output_dir, "EllipVsCFE_MargUtil")
